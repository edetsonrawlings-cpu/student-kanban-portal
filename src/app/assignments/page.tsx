...
const initialTasks: Task[] = [
  { id: "1", title: "Create Login Page & Database", course: "CS301 - Web Dev", dueDate: "Oct 24", priority: "High", status: "todo" },
  { id: "2", title: "Linear Systems Problem Set 4", course: "MATH214 - Linear Algebra", dueDate: "Oct 26", priority: "Medium", status: "todo" },
  { id: "3", title: "Implement Student Registration", course: "CS301 - Web Dev", dueDate: "Oct 28", priority: "High", status: "in-progress" },
  { id: "4", title: "Verify Payment Module", course: "CS301 - Web Dev", dueDate: "Oct 22", priority: "High", status: "testing" },
  { id: "5", title: "Design Homepage & Documentation", course: "CS301 - Web Dev", dueDate: "Oct 18", priority: "Low", status: "done" },
];
...
export default function KanbanPage() {
  ...
  const getNextStatus = (current: Status): Status => {
    switch (current) {
      ...
      case "in-progress": return "testing";
            ...
            >
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800/60 text-xs text-slate-500">
                ...
                <div className="flex gap-1">
                  {status !== "todo" && (
                    <button
                      onClick={() => moveTask(task.id, getPrevStatus(status))}
                  ...
                  {status !== "done" && (
                    <button
                      onClick={() => moveTask(task.id, getNextStatus(status))}
                      className="px-2 py-1 text-[11px] bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 rounded text-slate-700 dark:text-slate-300 transition"
                    >
                      →
                    </button>
